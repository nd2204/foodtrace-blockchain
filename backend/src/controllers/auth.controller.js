...
const authController = {
  register: async (req, res) => {
    ...
    try {
      ...
      await sendEmail(
        email,
        "Verify Your Account",
        `<p>Your verification code: <strong>${code}</strong> (expires in 10 minutes)</p>`,
      );
  ...
  forgotPassword: async (req, res) => {
    ...
    try {
      ...
      await sendEmail(
        email,
        "Password reset code",
        `<p>Your reset code: <strong>${code}</strong> (expires in 10 minutes)</p>`,
      );
  ...
  login: async (req, res) => {
    ...
    try {
      ...
      const jwtToken = tokenUtil.sign(payload, {
